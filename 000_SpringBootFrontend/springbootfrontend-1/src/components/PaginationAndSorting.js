...
const PaginationAndSorting = () => {
  ...
  const getSupplier = async () => {
    try {
      const responseData = await getSupplierService(); // Assuming testApiService returns a promise
      console.log(responseData);
      if (responseData.status === 200) {
        
        setProductList(responseData.data.data.content); // Store the 'data' part of the response
      } else {
  ...
  // Update rowsToShow whenever productList changes
  useEffect(() => {
  ...
  useEffect(() => {
    getSupplier(); // Call the TestApi function when the component mounts
